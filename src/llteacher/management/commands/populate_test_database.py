...
for category in categories:
    ...
    print(f"Most expensive {category}: {most_expensive['name']} (${most_expensive['price']})") """
        ...
        self.stdout.write(
            f"  ✓ Created {len(homeworks)} homeworks with sections and solutions"
        )
        return homeworks
    ...
    def create_conversations_and_messages(self, students, homeworks):
        ...
        for student in students:
            ...
            for homework in homeworks:
                sections_to_work_on.extend(
                    list(homework.sections.all())[:2]
                )  # First 2 sections of each homework

            for i, section in enumerate(
                sections_to_work_on[:3]
            ):  # Limit to 3 conversations per student
                conversation = Conversation.objects.create(
                    user=student.user, section=section
                )
                ...
                for j in range(
                    min(
                        len(template["student_messages"]), len(template["ai_responses"])
                    )
                ):
                    # Student message
        ...
        self.stdout.write(
            f"  ✓ Created {conversation_count} conversations with {message_count} messages"
        )
